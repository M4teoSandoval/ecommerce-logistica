    ...
    <div class="row g-4">
        <div class="col-md-5">
            <div class="content-card p-0 overflow-hidden">
                ...
                @else
                    <div
                        style="width:100%;height:380px;background:linear-gradient(135deg,#ede9fe,#dbeafe);display:flex;align-items:center;justify-content:center;">
        ...
        <div class="col-md-7">
            <div class="content-card">
                <span
                    style="background:#f1f5f9;color:#475569;padding:3px 10px;border-radius:6px;font-size:0.72rem;font-weight:600;">
                    {{ $producto->categoria }}
                </span>
                ...
                <div class="row g-2 mb-4">
                    @foreach ([['Peso', $producto->peso . ' kg', 'bi-box-seam'], ['Stock', $producto->stock . ' unidades', 'bi-layers'], ['Alto', $producto->alto ? $producto->alto . ' cm' : 'N/A', 'bi-arrows-vertical'], ['Ancho', $producto->ancho ? $producto->ancho . ' cm' : 'N/A', 'bi-arrows']] as $d)
                        <div class="col-6">
                            <div style="background:#f8fafc;border-radius:10px;padding:12px 14px;">
                                <div
                                    style="font-size:0.7rem;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">
                                    {{ $d[0] }}</div>
                ...
                @if ($producto->stock > 0)
                    <form method="POST" action="{{ route('cliente.carrito.agregar', $producto) }}">
                        @csrf
                        <div class="d-flex gap-2 mb-3">
                            <input type="number" name="cantidad" value="1" min="1" max="{{ $producto->stock }}"
                                class="form-control" style="width:80px;border-radius:10px;">
                            <button type="submit" class="btn btn-primary flex-1"
                                style="border-radius:10px;padding:10px 20px;font-size:0.9rem;font-weight:600;flex:1;">
